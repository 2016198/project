...
//レイ構造体
struct Ray {
	Position3 start;		//始点座標
	Vector3 vec;			//方向ベクトル
};

//基本図形となる基底クラス
struct Primitive {
	//ray	レイトレーシング用レイ構造体[in]
	//t		交点までの距離を返す用[out]
	//N		交点における表面の法線を返す用[out]
	virtual bool IshitRay(const Ray& ray, float& t, const Vector3& N)const = 0;
};
...
struct Sphere : public Primitive {
	...
	bool IshitRay(const Ray& ray, float& t, const Vector3& N)const override;
};
...
struct Plane : public Primitive {
	//これはax + by + cz = dを意味する
	//float a, b, c, d;
	Vector3 n;			//法線ベクトル(正規化しておいてください)
	float distance;		//原点からの距離
	Plane() : n(0.0f, 0.0f, 0.0f), distance(0.0f) {}
	Plane(Vector3& n, float d) : n(n), distance(d) {}
	bool IshitRay(const Ray& ray, float& t, const Vector3& N)const override;
};
