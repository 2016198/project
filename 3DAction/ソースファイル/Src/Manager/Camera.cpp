...
{
	//じゃけん、カメラ操作プログラム作りましょうね～
	VECTOR axisRad = AsoUtility::VECTOR_ZERO;
	if (CheckHitKey(KEY_INPUT_LEFT))
	{
		axisRad.y -= AsoUtility::Deg2RadF(1.0f);
	}
	...
	if (CheckHitKey(KEY_INPUT_UP) && mAngles.x < LIMIT_X_UP_RAD)
	{
		axisRad.x += AsoUtility::Deg2RadF(1.0f);
	}

	if (CheckHitKey(KEY_INPUT_DOWN) && mAngles.x > LIMIT_X_DW_RAD)
	{
		axisRad.x -= AsoUtility::Deg2RadF(1.0f);
	}

	if (!AsoUtility::EqualsVZero(axisRad))
	{
		mAngles = VAdd(mAngles, axisRad);
	}
